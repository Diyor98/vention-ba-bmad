...
| DESIGN-INT-5 | Host payouts | partial | small | Stat-card row above the table (Lifetime earnings / Pending payout / Next payout date) + Connect-status banner if `payouts_enabled = false`. `src/app/(owner)/owner/payouts/page.tsx`. |
| DESIGN-INT-6 | Host bookings tabs | partial | small | 4-tab strip with counts (Pending / Confirmed / Rejected / Cancelled). Needs a shared `<Tabs>` component or inline. `src/app/(owner)/owner/bookings/page.tsx`. |
| DESIGN-INT-7 | Host spaces list → table | partial | small | List → table (Space photo + name / City / Desks / Day rate / Status / Edit). `src/app/(owner)/owner/spaces/page.tsx`. |
...
| DESIGN-INT-12 | Become a host polish | partial | small | Story 7-3 state machine already in place. Compare prototype hero/wording to live; minor polish only. `src/app/become-a-host/page.tsx`. |
| DESIGN-INT-13 | Login + Signup polish | matched | small | Quick alignment pass; auth-card already there. Consider aliasing `/signup` → `/register` for prototype-shape compat. |
| DESIGN-INT-14 | Admin applications search + filter | matched | small | Story 7-4 reskin already aligned. Add search input + status filter chips. Verify note-on-reject mechanic matches prototype's ConfirmDialog shape. |
