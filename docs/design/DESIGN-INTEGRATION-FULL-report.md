...
| ~~DESIGN-INT-8~~ | ~~Host space edit sections~~ | **done** session 2 | medium | Shipped: rewrote `<EditSpaceForm>` (shared admin + owner Client Component) into 3 `.form-card` sections — Basics / Description & photo / Amenities. Replaced trailing button with `.save-bar` (sticky). Action / validation / variant-routing untouched. |
| ~~DESIGN-INT-9~~ | ~~Account page (greenfield)~~ | **done** session 2 | medium | Shipped: NEW `/account` route. Auth-gated (login redirect). 3 tabs via shared `<Tabs>`: Profile (read-only name + email from session), Payment (Phase 3 stub w/ disabled Add card affordance), Notifications (4-row toggle list, all Phase 3 stubs). |
| ~~DESIGN-INT-10~~ | ~~Host space new — 4-step wizard~~ | **done** session 2 (pragmatic shape) | medium | Shipped: visual stepper (1/2/3/4 → Basics / Desks / Photos / Publish) above existing `<CreateSpaceForm>`. **Documented judgment call**: full client-side wizard with deferred submission was out of scope ("preserve all existing data fetching, mutations, functionality") — backend splits space+desks across two server actions. Step 1 (Basics) saves; the user lands on the existing edit page (Step 2) where desks are added one at a time. Future story may unify into a true client-side state machine. |
| ~~DESIGN-INT-11~~ | ~~Host onboarding — 5-step wizard + success state~~ | **done** session 2 | large | Shipped: full restyle of `/owner/settings`. Pre-complete branch renders 2-col step-rail (5 steps with active/done states) + "Get started" body with 4-bullet KYC explainer + Lucide Check chips + "Continue with Stripe →" CTA. Complete branch renders the prototype's success card with 4 green capability indicators (Charges / Payouts / Identity / Tax form) + Manage on Stripe sidebar with "See payouts" link. data-testid attributes preserved. |
...
| ~~DESIGN-INT-15~~ | ~~Admin users — visual scaffold~~ | **done** session 2 | medium | Shipped: NEW `/admin/users` route. Read-only table (user + role + joined). Role-filter chips (All/Guest/Owner/Admin) + search input. New `listAllUsers` helper. Manage button rendered disabled with Phase 3 tooltip — no destructive actions wired. |
...
| ~~DESIGN-INT-18~~ | ~~Booking flow chrome~~ | **done** session 2 | small | Shipped: new `loading.tsx` for the booking return route. Renders Stripe-S icon + "checkout.stripe.com · Test mode" mini-chrome + spinner + "Finalizing your booking…" copy while the Server Component verifies the session. Real Stripe Checkout itself untouched (hosted page). |
