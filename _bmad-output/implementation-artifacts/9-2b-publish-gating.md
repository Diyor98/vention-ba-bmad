...
| 2026-05-18 | Story implemented; `spaces.status` enum extended to include `'DRAFT'` via migration `0004_fine_ronan.sql`; `createSpace` query helper gets optional `status` param; `createSpaceAction` passes `'DRAFT'` for SPACE_OWNER; new `publishSpaceAction` with 3-error-code typed return + 7-step locked behavior; list page gets inline Draft badge (no per-row Publish button per Decision §3); detail page gets Draft badge + new Client Component `<PublishSpaceButton>` (`useTransition` + toast pattern from Story 9-2); seed gets `owner-no-connect@deskhive.local` (bounded second seed user per Decision §5); Playwright fixture extended; 5 new unit tests + 2 new E2E tests. Memory entry extended. Single commit per AC-14 — awaiting BA browser walk before push. | `7e7251c` |
| 2026-05-18 | Post-BA-walk fixture-rename + test-owns-state refactor: BA browser walk verified the gated UI end-to-end by completing real Stripe Express test-mode onboarding for `owner-no-connect@deskhive.local`, leaving that user with an active `stripe_connect_accounts` row + a published space. Three changes: (1) renamed seed user → `owner-pending-onboarding@deskhive.local` + added `scrubPendingOnboardingConnectRow` step that runs every `pnpm db:seed`. (2) Refactored the publish-gating E2E to have BOTH tests sign in as the pending-onboarding user and let the test itself own the Connect-row lifecycle (happy path INSERTs a synthetic row mid-test; gated path leaves it absent) — eliminates the original cross-file race against `connect-onboarding.spec.ts` and is the canonical template for Stories 9-3 / 9-4 / 9-6. (3) Added `test.describe.configure({ mode: 'serial' })` + exact-name space cleanup to eliminate intra-file parallelism races on the shared user. The pre-rename `owner-no-connect@deskhive.local` user is intentionally LEFT in place as a harmless orphan (auto-deletion would cascade-orphan the BA's published space — `spaces.ownerId` has no `onDelete: cascade`). BA decisions doc Decision §5 expanded with the rationale + naming history + the test-owns-state + serial-mode + exact-name patterns. Memory file updated with the canonical Theme B gated-action E2E pattern. Both publish-gating tests pass in isolation AND under full-suite parallel. | `TBD (this follow-up)` |
