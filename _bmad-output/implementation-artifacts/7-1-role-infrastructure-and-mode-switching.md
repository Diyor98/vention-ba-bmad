...
| 2026-05-12 | Story implemented; schema + migration applied to Neon, mode helpers + `switchModeAction` shipped, Header refactored to 5 variants, `<UserPill>` dropdown + `/owner/*` placeholders landed, seed extended with SPACE_OWNER. One mid-implementation pivot (Debug Log #1) moved const exports out of the `'use server'` file. Single commit per AC-17. | (filled by a small follow-up commit after push, once the hash is stable — same pattern as Stories 5.1 / 5.2 / 6.1 / 6.2 / 6.3 / 6.6) |
